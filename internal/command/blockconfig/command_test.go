...
func TestNativeCommand(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cfg, err := New(blocks[0])
			require.NoError(t, err)

			options := &command.NativeCommandOptions{
				Env:    tc.env,
				Stdout: stdout,
				Stderr: stderr,
				Stdin:  tc.input,
			...
			command, err := command.NewNative(cfg, options)
			require.NoError(t, err)
...
func TestVirtualCommand(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			options := &command.VirtualCommandOptions{
				Env:    tc.env,
				Stdout: stdout,
				Stdin:  tc.input,
				Logger: logger,
			}
			command, err := command.NewVirtual(cfg, options)
			require.NoError(t, err)
...
func TestVirtualCommandInputUsingPipe(t *testing.T) {
	...
	t.Run("Cat", func(t *testing.T) {
		...
		remoteOptions := &command.VirtualCommandOptions{
			Stdin:  stdinR,
			Stdout: stdout,
			Logger: logger,
		}
		command, err := command.NewVirtual(cfg, remoteOptions)
		require.NoError(t, err)
